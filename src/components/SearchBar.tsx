...
export default function SearchBar(props: SearchType) {
    ...
    const onChangeInput = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const _matched = recipes.map(item => item.toLowerCase()).filter((item) => item.includes(recipe.toLowerCase()));
            ...
            <ul className={`recipe-list-container ${showDishList ? '' : 'hide'}`}>
                {
                    recipes.map((item, index) => (
                        matchedRecipes.includes(item.toLowerCase()) ? <li onClick={() => selecteRecipeAction(index)} key={index}>{item}</li> : null
                    ))
                }
            </ul>
        </div>
